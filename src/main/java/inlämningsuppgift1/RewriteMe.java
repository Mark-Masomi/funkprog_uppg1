...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return (int) questions.stream().filter(e->e.getCategory()
                .equals(category)).count();
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(e->e.getCategory().equals(category)).map(Question::getQuestionString)
                .collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream().map(Question::getAllAnswers).flatMap(Collection::stream).distinct()
                .collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().anyMatch(e->e.getAllAnswers().contains(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().filter(e->e.getAllAnswers().contains(answerCandidate)).count();
    }
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        return questions.stream().collect(Collectors.groupingBy(Question::getCategory,
                Collectors.mapping(e->e.getQuestionString(),Collectors.toList())));
    }
    ...
    {
        return questions.stream().filter(e->e.getCategory().equals(c)).map(m->m.getAllAnswers())
